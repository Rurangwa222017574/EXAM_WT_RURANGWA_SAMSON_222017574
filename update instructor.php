...
if (isset($_REQUEST['id'])) {
    ...
    $stmt = $con->prepare("SELECT * FROM instructor WHERE instructor_id=?");
    ...
    if ($result->num_rows > 0) {
        ...
        echo "Instructor record not found.";
    }
}
...
<body>
    ...
    <div class="col-6 ">
    <h2>Update Form of Instructor</h2>
        <form  method="POST" action="" >
          <div class="mb-3">
            <label for="First-name" class="col-form-label">FirstName:</label>
            <input type="text" class="form-control" name="First-name" value="<?php echo $row["instructor_Fname"]; ?>">
          </div>
          <div class="mb-3">
            <label for="Last-name" class="col-form-label">LastName:</label>
            <input type="text" class="form-control" name="Last-name" value="<?php echo $row["instructor_Lname"]; ?>">
          </div>
          <div class="mb-3">
            ...
            <select class="form-control" id="message-text" name="Course-name">
                ...
                <option  value="<?php echo $courses['course_id']; ?>" <?php echo ($cou==$courses['course_id'])? 'selected' :''; ?> ><?php echo $courses['course_name'];?></option>
                <?php
                }
                ?>
            </select>
...
if (isset($_POST['update'])) {
    ...
    // Update the instructor record in the database (prepared statement again for security)
    $stmt = mysqli_query($con,"UPDATE instructor SET instructor_Fname='$Fname',instructor_Lname='$Lname',course_id='$Cname' WHERE instructor_id='$instructor_id'");
    ?>
    <script>
